...
def ori_callback(msg):
    ori_data[0] = msg.x
    ori_data[1] = msg.y -  gain_ctrl.linear.x * np.pi /180
...
def stand(ik):
    global JOINTS, finish, step, delta_step, COM, LEFT, RIGHT
    COM = np.matrix([X_OFFSET, 0.0, COM_HEIGHT])
    LEFT = np.matrix([0.0, 38.5 / 1000 + FOOT_DISTANCE, 0.0])
    RIGHT = np.matrix([0.0, -38.5 / 1000 - FOOT_DISTANCE, 0.0])
...
def push_test(fz, ik):
    global flag, finish, delta_step,step, start, now, JOINTS, COM, LEFT, RIGHT, uCOM, uLEFT, uRIGHT
    time_step = .1

    if finish == False:

        if (ori_data[1] * 180 / np.pi >= -20 and gyr_data[1] * 180 / np.pi <= 20) and not(step):
            print("IN 1")
            JOINTS = ik.solve(COM, LEFT, RIGHT)
...
def final_test(pc, fz, ik):
    ...
    if finish == False:
        if pc.walking_ready and not debug:
            if (ori_data[1] * 180 / np.pi >= -35 and gyr_data[1] * 180 / np.pi <= 33) and not(step):
                print("WALK")
                JOINTS = ik.solve(pc.com_pose, pc.l_foot, pc.r_foot)
                start = rospy.Time.now().to_sec()
                delta_step = fz.compute(-ori_data[1] * 180 /
